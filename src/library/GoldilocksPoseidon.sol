...
library GoldilocksPoseidon {
    ...
    function _mdsRowShf0(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            // res = add(res, state[r] * MDS_MATRIX_DIAG[r]); // 200 bits
            res = state[1] * MDS_MATRIX_CIRC_1 + state[2] * MDS_MATRIX_CIRC_2 + (state[3] << 4) // state[3] * MDS_MATRIX_CIRC_3
                + state[4] * MDS_MATRIX_CIRC_4 + state[5] * MDS_MATRIX_CIRC_5 + state[6] * MDS_MATRIX_CIRC_6
                + state[7] * MDS_MATRIX_CIRC_7 + state[8] * MDS_MATRIX_CIRC_8 + state[9] * MDS_MATRIX_CIRC_9
                + state[10] * MDS_MATRIX_CIRC_10 + state[11] * MDS_MATRIX_CIRC_11
    ...
    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf1(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[1] * MDS_MATRIX_CIRC_0 + state[2] * MDS_MATRIX_CIRC_1 + state[3] * MDS_MATRIX_CIRC_2
                + (state[4] << 4) // state[4] * MDS_MATRIX_CIRC_3
                + state[5] * MDS_MATRIX_CIRC_4 + state[6] * MDS_MATRIX_CIRC_5 + state[7] * MDS_MATRIX_CIRC_6
                + state[8] * MDS_MATRIX_CIRC_7 + state[9] * MDS_MATRIX_CIRC_8 + state[10] * MDS_MATRIX_CIRC_9
                + state[11] * MDS_MATRIX_CIRC_10 + state[0] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf2(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[2] * MDS_MATRIX_CIRC_0 + state[3] * MDS_MATRIX_CIRC_1 + state[4] * MDS_MATRIX_CIRC_2
                + (state[5] << 4) // state[5] * MDS_MATRIX_CIRC_3
                + state[6] * MDS_MATRIX_CIRC_4 + state[7] * MDS_MATRIX_CIRC_5 + state[8] * MDS_MATRIX_CIRC_6
                + state[9] * MDS_MATRIX_CIRC_7 + state[10] * MDS_MATRIX_CIRC_8 + state[11] * MDS_MATRIX_CIRC_9
                + state[0] * MDS_MATRIX_CIRC_10 + state[1] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf3(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[3] * MDS_MATRIX_CIRC_0 + state[4] * MDS_MATRIX_CIRC_1 + state[5] * MDS_MATRIX_CIRC_2
                + (state[6] << 4) // state[6] * MDS_MATRIX_CIRC_3
                + state[7] * MDS_MATRIX_CIRC_4 + state[8] * MDS_MATRIX_CIRC_5 + state[9] * MDS_MATRIX_CIRC_6
                + state[10] * MDS_MATRIX_CIRC_7 + state[11] * MDS_MATRIX_CIRC_8 + state[0] * MDS_MATRIX_CIRC_9
                + state[1] * MDS_MATRIX_CIRC_10 + state[2] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf4(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[4] * MDS_MATRIX_CIRC_0 + state[5] * MDS_MATRIX_CIRC_1 + state[6] * MDS_MATRIX_CIRC_2
                + (state[7] << 4) // state[7] * MDS_MATRIX_CIRC_3
                + state[8] * MDS_MATRIX_CIRC_4 + state[9] * MDS_MATRIX_CIRC_5 + state[10] * MDS_MATRIX_CIRC_6
                + state[11] * MDS_MATRIX_CIRC_7 + state[0] * MDS_MATRIX_CIRC_8 + state[1] * MDS_MATRIX_CIRC_9
                + state[2] * MDS_MATRIX_CIRC_10 + state[3] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf5(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[5] * MDS_MATRIX_CIRC_0 + state[6] * MDS_MATRIX_CIRC_1 + state[7] * MDS_MATRIX_CIRC_2
                + (state[8] << 4) // state[8] * MDS_MATRIX_CIRC_3
                + state[9] * MDS_MATRIX_CIRC_4 + state[10] * MDS_MATRIX_CIRC_5 + state[11] * MDS_MATRIX_CIRC_6
                + state[0] * MDS_MATRIX_CIRC_7 + state[1] * MDS_MATRIX_CIRC_8 + state[2] * MDS_MATRIX_CIRC_9
                + state[3] * MDS_MATRIX_CIRC_10 + state[4] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf6(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[6] * MDS_MATRIX_CIRC_0 + state[7] * MDS_MATRIX_CIRC_1 + state[8] * MDS_MATRIX_CIRC_2
                + (state[9] << 4) // state[9] * MDS_MATRIX_CIRC_3
                + state[10] * MDS_MATRIX_CIRC_4 + state[11] * MDS_MATRIX_CIRC_5 + state[0] * MDS_MATRIX_CIRC_6
                + state[1] * MDS_MATRIX_CIRC_7 + state[2] * MDS_MATRIX_CIRC_8 + state[3] * MDS_MATRIX_CIRC_9
                + state[4] * MDS_MATRIX_CIRC_10 + state[5] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf7(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[7] * MDS_MATRIX_CIRC_0 + state[8] * MDS_MATRIX_CIRC_1 + state[9] * MDS_MATRIX_CIRC_2
                + (state[10] << 4) // state[10] * MDS_MATRIX_CIRC_3
                + state[11] * MDS_MATRIX_CIRC_4 + state[0] * MDS_MATRIX_CIRC_5 + state[1] * MDS_MATRIX_CIRC_6
                + state[2] * MDS_MATRIX_CIRC_7 + state[3] * MDS_MATRIX_CIRC_8 + state[4] * MDS_MATRIX_CIRC_9
                + state[5] * MDS_MATRIX_CIRC_10 + state[6] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf8(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[8] * MDS_MATRIX_CIRC_0 + state[9] * MDS_MATRIX_CIRC_1 + state[10] * MDS_MATRIX_CIRC_2
                + (state[11] << 4) // state[11] * MDS_MATRIX_CIRC_3
                + state[0] * MDS_MATRIX_CIRC_4 + state[1] * MDS_MATRIX_CIRC_5 + state[2] * MDS_MATRIX_CIRC_6
                + state[3] * MDS_MATRIX_CIRC_7 + state[4] * MDS_MATRIX_CIRC_8 + state[5] * MDS_MATRIX_CIRC_9
                + state[6] * MDS_MATRIX_CIRC_10 + state[7] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf9(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[9] * MDS_MATRIX_CIRC_0 + state[10] * MDS_MATRIX_CIRC_1 + state[11] * MDS_MATRIX_CIRC_2
                + (state[0] << 4) // state[0] * MDS_MATRIX_CIRC_3
                + state[1] * MDS_MATRIX_CIRC_4 + state[2] * MDS_MATRIX_CIRC_5 + state[3] * MDS_MATRIX_CIRC_6
                + state[4] * MDS_MATRIX_CIRC_7 + state[5] * MDS_MATRIX_CIRC_8 + state[6] * MDS_MATRIX_CIRC_9
                + state[7] * MDS_MATRIX_CIRC_10 + state[8] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf10(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[10] * MDS_MATRIX_CIRC_0 + state[11] * MDS_MATRIX_CIRC_1 + state[0] * MDS_MATRIX_CIRC_2
                + (state[1] << 4) // state[1] * MDS_MATRIX_CIRC_3
                + state[2] * MDS_MATRIX_CIRC_4 + state[3] * MDS_MATRIX_CIRC_5 + state[4] * MDS_MATRIX_CIRC_6
                + state[5] * MDS_MATRIX_CIRC_7 + state[6] * MDS_MATRIX_CIRC_8 + state[7] * MDS_MATRIX_CIRC_9
                + state[8] * MDS_MATRIX_CIRC_10 + state[9] * MDS_MATRIX_CIRC_11;
        }
    }

    // `state[r]` allows 192 bits number.
    // `res` is 200 bits number.
    function _mdsRowShf11(uint256[WIDTH] memory state) internal pure returns (uint256 res) {
        unchecked {
            // for (uint256 i = 0; i < 12; i++) {
            //     res += state[(i + r) % WIDTH] * MDS_MATRIX_CIRC[i]; // (192 + 8) bits
            // }
            res = state[11] * MDS_MATRIX_CIRC_0 + state[0] * MDS_MATRIX_CIRC_1 + state[1] * MDS_MATRIX_CIRC_2
                + (state[2] << 4) // state[2] * MDS_MATRIX_CIRC_3
                + state[3] * MDS_MATRIX_CIRC_4 + state[4] * MDS_MATRIX_CIRC_5 + state[5] * MDS_MATRIX_CIRC_6
                + state[6] * MDS_MATRIX_CIRC_7 + state[7] * MDS_MATRIX_CIRC_8 + state[8] * MDS_MATRIX_CIRC_9
                + state[9] * MDS_MATRIX_CIRC_10 + state[10] * MDS_MATRIX_CIRC_11;
        }
    }

    function _mdsPartialLayerInit1(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0x80772dc2645b280b + state[2] * 0xe796d293a47a64cb + state[3] * 0xdcedab70f40718ba
                + state[4] * 0xf4a437f2888ae909 + state[5] * 0xf97abba0dffb6c50 + state[6] * 0x7f8e41e0b0a6cdff
                + state[7] * 0x726af914971c1374 + state[8] * 0x64dd936da878404d + state[9] * 0x85418a9fef8a9890
                + state[10] * 0x156048ee7a738154 + state[11] * 0xd841e8ef9dde8ba0;
        }
    }

    function _mdsPartialLayerInit2(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0xdc927721da922cf8 + state[2] * 0xb124c33152a2421a + state[3] * 0x14a4a64da0b2668f
                + state[4] * 0xc537d44dc2875403 + state[5] * 0x5e40f0c9bb82aab5 + state[6] * 0x4b1ba8d40afca97d
                + state[7] * 0x1d7f8a2cce1a9d00 + state[8] * 0x4db9a2ead2bd7262 + state[9] * 0xd8a2eb7ef5e707ad
                + state[10] * 0x91f7562377e81df5 + state[11] * 0x156048ee7a738154;
        }
    }

    function _mdsPartialLayerInit3(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0xc1978156516879ad + state[2] * 0x0ee5dc0ce131268a + state[3] * 0x4715b8e5ab34653b
                + state[4] * 0x7f68007619fd8ba9 + state[5] * 0x5996a80497e24a6b + state[6] * 0x623708f28fca70e8
                + state[7] * 0x18737784700c75cd + state[8] * 0xbe2e19f6d07f1a83 + state[9] * 0xbfe85ababed2d882
                + state[10] * 0xd8a2eb7ef5e707ad + state[11] * 0x85418a9fef8a9890;
        }
    }

    function _mdsPartialLayerInit4(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0x90e80c591f48b603 + state[2] * 0xa9032a52f930fae6 + state[3] * 0x1e8916a99c93a88e
                + state[4] * 0xa4911db6a32612da + state[5] * 0x07084430a7307c9a + state[6] * 0xbf150dc4914d380f
                + state[7] * 0x7fb45d605dd82838 + state[8] * 0x02290fe23c20351a + state[9] * 0xbe2e19f6d07f1a83
                + state[10] * 0x4db9a2ead2bd7262 + state[11] * 0x64dd936da878404d;
        }
    }

    function _mdsPartialLayerInit5(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0x3a2432625475e3ae + state[2] * 0x7e33ca8c814280de + state[3] * 0xbba4b5d86b9a3b2c
                + state[4] * 0x2f7e9aade3fdaec1 + state[5] * 0xad2f570a5b8545aa + state[6] * 0xc26a083554767106
                + state[7] * 0x862361aeab0f9b6e + state[8] * 0x7fb45d605dd82838 + state[9] * 0x18737784700c75cd
                + state[10] * 0x1d7f8a2cce1a9d00 + state[11] * 0x726af914971c1374;
        }
    }

    function _mdsPartialLayerInit6(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0x00a2d4321cca94fe + state[2] * 0xad11180f69a8c29e + state[3] * 0xe76649f9bd5d5c2e
                + state[4] * 0xe7ffd578da4ea43d + state[5] * 0xab7f81fef4274770 + state[6] * 0x753b8b1126665c22
                + state[7] * 0xc26a083554767106 + state[8] * 0xbf150dc4914d380f + state[9] * 0x623708f28fca70e8
                + state[10] * 0x4b1ba8d40afca97d + state[11] * 0x7f8e41e0b0a6cdff;
        }
    }

    function _mdsPartialLayerInit7(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0x77736f524010c932 + state[2] * 0xc75ac6d5b5a10ff3 + state[3] * 0xaf8e2518a1ece54d
                + state[4] * 0x43a608e7afa6b5c2 + state[5] * 0xcb81f535cf98c9e9 + state[6] * 0xab7f81fef4274770
                + state[7] * 0xad2f570a5b8545aa + state[8] * 0x07084430a7307c9a + state[9] * 0x5996a80497e24a6b
                + state[10] * 0x5e40f0c9bb82aab5 + state[11] * 0xf97abba0dffb6c50;
        }
    }

    function _mdsPartialLayerInit8(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0x904d3f2804a36c54 + state[2] * 0xf0674a8dc5a387ec + state[3] * 0xdcda1344cdca873f
                + state[4] * 0xca46546aa99e1575 + state[5] * 0x43a608e7afa6b5c2 + state[6] * 0xe7ffd578da4ea43d
                + state[7] * 0x2f7e9aade3fdaec1 + state[8] * 0xa4911db6a32612da + state[9] * 0x7f68007619fd8ba9
                + state[10] * 0xc537d44dc2875403 + state[11] * 0xf4a437f2888ae909;
        }
    }

    function _mdsPartialLayerInit9(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0xbf9b39e28a16f354 + state[2] * 0xb36d43120eaa5e2b + state[3] * 0xcd080204256088e5
                + state[4] * 0xdcda1344cdca873f + state[5] * 0xaf8e2518a1ece54d + state[6] * 0xe76649f9bd5d5c2e
                + state[7] * 0xbba4b5d86b9a3b2c + state[8] * 0x1e8916a99c93a88e + state[9] * 0x4715b8e5ab34653b
                + state[10] * 0x14a4a64da0b2668f + state[11] * 0xdcedab70f40718ba;
        }
    }

    function _mdsPartialLayerInit10(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0x3a1ded54a6cd058b + state[2] * 0x6f232aab4b533a25 + state[3] * 0xb36d43120eaa5e2b
                + state[4] * 0xf0674a8dc5a387ec + state[5] * 0xc75ac6d5b5a10ff3 + state[6] * 0xad11180f69a8c29e
                + state[7] * 0x7e33ca8c814280de + state[8] * 0xa9032a52f930fae6 + state[9] * 0x0ee5dc0ce131268a
                + state[10] * 0xb124c33152a2421a + state[11] * 0xe796d293a47a64cb;
        }
    }

    function _mdsPartialLayerInit11(uint256[WIDTH] memory state) private pure returns (uint256 res) {
        unchecked {
            res = state[1] * 0x42392870da5737cf + state[2] * 0x3a1ded54a6cd058b + state[3] * 0xbf9b39e28a16f354
                + state[4] * 0x904d3f2804a36c54 + state[5] * 0x77736f524010c932 + state[6] * 0x00a2d4321cca94fe
                + state[7] * 0x3a2432625475e3ae + state[8] * 0x90e80c591f48b603 + state[9] * 0xc1978156516879ad
                + state[10] * 0xdc927721da922cf8 + state[11] * 0x80772dc2645b280b;
        }
    }
    ...
    function _mdsPartialLayerFast(uint256[WIDTH] memory state, uint256 r)
        internal
        pure
        returns (uint256[WIDTH] memory newState)
    {
        unchecked {
            ...
            // }
            if (r == 0) {
                dSum += state[1] * 0x3d999c961b7c63b0 + state[2] * 0x814e82efcd172529 + state[3] * 0x2421e5d236704588
                    + state[4] * 0x887af7d4dd482328 + state[5] * 0xa5e9c291f6119b27 + state[6] * 0xbdc52b2676a4b4aa
                    + state[7] * 0x64832009d29bcf57 + state[8] * 0x09c4155174a552cc + state[9] * 0x463f9ee03d290810
                    + state[10] * 0xc810936e64982542 + state[11] * 0x043b1c289f7bc3ac;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x673655aae8be5a8b + state[2] * 0xd510fe714f39fa10 + state[3] * 0x2c68a099b51c9e73
                    + state[4] * 0xa667bfa9aa96999d + state[5] * 0x4d67e72f063e2108 + state[6] * 0xf84dde3e6acda179
                    + state[7] * 0x40f9cc8c08f80981 + state[8] * 0x5ead032050097142 + state[9] * 0x6591b02092d671bb
                    + state[10] * 0x00e18c71963dd1b7 + state[11] * 0x8a21bcd24a14218a;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x202800f4addbdc87 + state[2] * 0xe4b5bdb1cc3504ff + state[3] * 0xbe32b32a825596e7
                    + state[4] * 0x8e0f68c5dc223b9a + state[5] * 0x58022d9e1c256ce3 + state[6] * 0x584d29227aa073ac
                    + state[7] * 0x8b9352ad04bef9e7 + state[8] * 0xaead42a3f445ecbf + state[9] * 0x3c667a1d833a3cca
                    + state[10] * 0xda6f61838efa1ffe + state[11] * 0xe8f749470bd7c446;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0xc5b85bab9e5b3869 + state[2] * 0x45245258aec51cf7 + state[3] * 0x16e6b8e68b931830
                    + state[4] * 0xe2ae0f051418112c + state[5] * 0x0470e26a0093a65b + state[6] * 0x6bef71973a8146ed
                    + state[7] * 0x119265be51812daf + state[8] * 0xb0be7356254bea2e + state[9] * 0x8584defff7589bd7
                    + state[10] * 0x3c5fe4aeb1fb52ba + state[11] * 0x9e7cd88acf543a5e;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x179be4bba87f0a8c + state[2] * 0xacf63d95d8887355 + state[3] * 0x6696670196b0074f
                    + state[4] * 0xd99ddf1fe75085f9 + state[5] * 0xc2597881fef0283b + state[6] * 0xcf48395ee6c54f14
                    + state[7] * 0x15226a8e4cd8d3b6 + state[8] * 0xc053297389af5d3b + state[9] * 0x2c08893f0d1580e2
                    + state[10] * 0x0ed3cbcff6fcc5ba + state[11] * 0xc82f510ecf81f6d0;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x94b06183acb715cc + state[2] * 0x500392ed0d431137 + state[3] * 0x861cc95ad5c86323
                    + state[4] * 0x05830a443f86c4ac + state[5] * 0x3b68225874a20a7c + state[6] * 0x10b3309838e236fb
                    + state[7] * 0x9b77fc8bcd559e2c + state[8] * 0xbdecf5e0cb9cb213 + state[9] * 0x30276f1221ace5fa
                    + state[10] * 0x7935dd342764a144 + state[11] * 0xeac6db520bb03708;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x7186a80551025f8f + state[2] * 0x622247557e9b5371 + state[3] * 0xc4cbe326d1ad9742
                    + state[4] * 0x55f1523ac6a23ea2 + state[5] * 0xa13dfe77a3d52f53 + state[6] * 0xe30750b6301c0452
                    + state[7] * 0x08bd488070a3a32b + state[8] * 0xcd800caef5b72ae3 + state[9] * 0x83329c90f04233ce
                    + state[10] * 0xb5b99e6664a0a3ee + state[11] * 0x6b0731849e200a7f;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0xec3fabc192b01799 + state[2] * 0x382b38cee8ee5375 + state[3] * 0x3bfb6c3f0e616572
                    + state[4] * 0x514abd0cf6c7bc86 + state[5] * 0x47521b1361dcc546 + state[6] * 0x178093843f863d14
                    + state[7] * 0xad1003c5d28918e7 + state[8] * 0x738450e42495bc81 + state[9] * 0xaf947c59af5e4047
                    + state[10] * 0x4653fb0685084ef2 + state[11] * 0x057fde2062ae35bf;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0xe376678d843ce55e + state[2] * 0x66f3860d7514e7fc + state[3] * 0x7817f3dfff8b4ffa
                    + state[4] * 0x3929624a9def725b + state[5] * 0x0126ca37f215a80a + state[6] * 0xfce2f5d02762a303
                    + state[7] * 0x1bc927375febbad7 + state[8] * 0x85b481e5243f60bf + state[9] * 0x2d3c5f42a39c91a0
                    + state[10] * 0x0811719919351ae8 + state[11] * 0xf669de0add993131;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x7de38bae084da92d + state[2] * 0x5b848442237e8a9b + state[3] * 0xf6c705da84d57310
                    + state[4] * 0x31e6a4bdb6a49017 + state[5] * 0x889489706e5c5c0f + state[6] * 0x0e4a205459692a1b
                    + state[7] * 0xbac3fa75ee26f299 + state[8] * 0x5f5894f4057d755e + state[9] * 0xb0dc3ecd724bb076
                    + state[10] * 0x5e34d8554a6452ba + state[11] * 0x04f78fd8c1fdcc5f;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x4dd19c38779512ea + state[2] * 0xdb79ba02704620e9 + state[3] * 0x92a29a3675a5d2be
                    + state[4] * 0xd5177029fe495166 + state[5] * 0xd32b3298a13330c1 + state[6] * 0x251c4a3eb2c5f8fd
                    + state[7] * 0xe1c48b26e0d98825 + state[8] * 0x3301d3362a4ffccb + state[9] * 0x09bb6c88de8cd178
                    + state[10] * 0xdc05b676564f538a + state[11] * 0x60192d883e473fee;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x16b9774801ac44a0 + state[2] * 0x3cb8411e786d3c8e + state[3] * 0xa86e9cf505072491
                    + state[4] * 0x0178928152e109ae + state[5] * 0x5317b905a6e1ab7b + state[6] * 0xda20b3be7f53d59f
                    + state[7] * 0xcb97dedecebee9ad + state[8] * 0x4bd545218c59f58d + state[9] * 0x77dc8d856c05a44a
                    + state[10] * 0x87948589e4f243fd + state[11] * 0x7e5217af969952c2;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0xbc58987d06a84e4d + state[2] * 0x0b5d420244c9cae3 + state[3] * 0xa3c4711b938c02c0
                    + state[4] * 0x3aace640a3e03990 + state[5] * 0x865a0f3249aacd8a + state[6] * 0x8d00b2a7dbed06c7
                    + state[7] * 0x6eacb905beb7e2f8 + state[8] * 0x045322b216ec3ec7 + state[9] * 0xeb9de00d594828e6
                    + state[10] * 0x088c5f20df9e5c26 + state[11] * 0xf555f4112b19781f;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0xa8cedbff1813d3a7 + state[2] * 0x50dcaee0fd27d164 + state[3] * 0xf1cb02417e23bd82
                    + state[4] * 0xfaf322786e2abe8b + state[5] * 0x937a4315beb5d9b6 + state[6] * 0x1b18992921a11d85
                    + state[7] * 0x7d66c4368b3c497b + state[8] * 0x0e7946317a6b4e99 + state[9] * 0xbe4430134182978b
                    + state[10] * 0x3771e82493ab262d + state[11] * 0xa671690d8095ce82;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0xb035585f6e929d9d + state[2] * 0xba1579c7e219b954 + state[3] * 0xcb201cf846db4ba3
                    + state[4] * 0x287bf9177372cf45 + state[5] * 0xa350e4f61147d0a6 + state[6] * 0xd5d0ecfb50bcff99
                    + state[7] * 0x2e166aa6c776ed21 + state[8] * 0xe1e66c991990e282 + state[9] * 0x662b329b01e7bb38
                    + state[10] * 0x8aa674b36144d9a9 + state[11] * 0xcbabf78f97f95e65;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0xeec24b15a06b53fe + state[2] * 0xc8a7aa07c5633533 + state[3] * 0xefe9c6fa4311ad51
                    + state[4] * 0xb9173f13977109a1 + state[5] * 0x69ce43c9cc94aedc + state[6] * 0xecf623c9cd118815
                    + state[7] * 0x28625def198c33c7 + state[8] * 0xccfc5f7de5c3636a + state[9] * 0xf5e6c40f1621c299
                    + state[10] * 0xcec0e58c34cb64b1 + state[11] * 0xa868ea113387939f;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0xd8dddbdc5ce4ef45 + state[2] * 0xacfc51de8131458c + state[3] * 0x146bb3c0fe499ac0
                    + state[4] * 0x9e65309f15943903 + state[5] * 0x80d0ad980773aa70 + state[6] * 0xf97817d4ddbf0607
                    + state[7] * 0xe4626620a75ba276 + state[8] * 0x0dfdc7fd6fc74f66 + state[9] * 0xf464864ad6f2bb93
                    + state[10] * 0x02d55e52a5d44414 + state[11] * 0xdd8de62487c40925;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0xc15acf44759545a3 + state[2] * 0xcbfdcf39869719d4 + state[3] * 0x33f62042e2f80225
                    + state[4] * 0x2599c5ead81d8fa3 + state[5] * 0x0b306cb6c1d7c8d0 + state[6] * 0x658c80d3df3729b1
                    + state[7] * 0xe8d1b2b21b41429c + state[8] * 0xa1b67f09d4b3ccb8 + state[9] * 0x0e1adf8b84437180
                    + state[10] * 0x0d593a5e584af47b + state[11] * 0xa023d94c56e151c7;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x49026cc3a4afc5a6 + state[2] * 0xe06dff00ab25b91b + state[3] * 0x0ab38c561e8850ff
                    + state[4] * 0x92c3c8275e105eeb + state[5] * 0xb65256e546889bd0 + state[6] * 0x3c0468236ea142f6
                    + state[7] * 0xee61766b889e18f2 + state[8] * 0xa206f41b12c30415 + state[9] * 0x02fe9d756c9f12d1
                    + state[10] * 0xe9633210630cbf12 + state[11] * 0x1ffea9fe85a0b0b1;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x81d1ae8cc50240f3 + state[2] * 0xf4c77a079a4607d7 + state[3] * 0xed446b2315e3efc1
                    + state[4] * 0x0b0a6b70915178c3 + state[5] * 0xb11ff3e089f15d9a + state[6] * 0x1d4dba0b7ae9cc18
                    + state[7] * 0x65d74e2f43b48d05 + state[8] * 0xa2df8c6b8ae0804a + state[9] * 0xa4e6f0a8c33348a6
                    + state[10] * 0xc0a26efc7be5669b + state[11] * 0xa6b6582c547d0d60;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x84afc741f1c13213 + state[2] * 0x2f8f43734fc906f3 + state[3] * 0xde682d72da0a02d9
                    + state[4] * 0x0bb005236adb9ef2 + state[5] * 0x5bdf35c10a8b5624 + state[6] * 0x0739a8a343950010
                    + state[7] * 0x52f515f44785cfbc + state[8] * 0xcbaf4e5d82856c60 + state[9] * 0xac9ea09074e3e150
                    + state[10] * 0x8f0fa011a2035fb0 + state[11] * 0x1a37905d8450904a;
                newState[0] = mod(dSum);
                ...
                dSum += state[1] * 0x3abeb80def61cc85 + state[2] * 0x9d19c9dd4eac4133 + state[3] * 0x075a652d9641a985
                    + state[4] * 0x9daf69ae1b67e667 + state[5] * 0x364f71da77920a18 + state[6] * 0x50bd769f745c95b1
                    + state[7] * 0xf223d1180dbbf3fc + state[8] * 0x2f885e584e04aa99 + state[9] * 0xb69a0fa70aea684a
                    + state[10] * 0x09584acaa6e062a0 + state[11] * 0x0bc051640145b19b;
                newState[0] = mod(dSum);
    ...
    function _partialFirstLayer(uint256[WIDTH] memory state) internal pure returns (uint256[WIDTH] memory newState) {
    ...
    // `newState[i]` is 64 bits number.
    function _constantLayer(uint256[WIDTH] memory state, uint256 roundCtr)
        internal
        pure
        returns (uint256[WIDTH] memory newState)
    {
    ...
    function _mdsSboxLayer(uint256[WIDTH] memory state) internal pure returns (uint256[WIDTH] memory newState) {
        // _sbox_layer
        // for (uint256 i = 0; i < 12; i++) {
    ...
    function permute(uint256[WIDTH] memory state) external pure returns (uint256[WIDTH] memory newState) {
    ...
    function _hashNToMNoPad(uint256[] memory input, uint256 numOutputs)
        internal
        pure
    ...
    function hashNToMNoPad(uint256[] memory input, uint256 numOutputs)
        external
        pure
        returns (uint256[] memory output)
    {
    ...
    function decodeHashOut(bytes32 encoded) public pure returns (uint256[4] memory output) {
    ...
    function encodeHashOut(uint256[4] memory value) public pure returns (bytes32 encoded) {
        encoded = abi.decode(
            abi.encodePacked(uint64(value[3]), uint64(value[2]), uint64(value[1]), uint64(value[0])), (bytes32)
        );
    }

    function twoToOne(bytes32 left, bytes32 right) public pure returns (bytes32 output) {
